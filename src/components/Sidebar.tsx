...
import { FileText, ListTodo, Folder as FolderIcon, File, Shield, Plus, LogOut, ChevronRight, ChevronDown, Pencil, Trash2, Calendar } from "lucide-react";
...
export function Sidebar({
  ...
  return (
    <aside
      className="w-72 shrink-0 h-screen sticky top-0 flex flex-col border-r"
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 text-sm">
        ...
        <div className="mb-6">
          ...
          {rootNotes.map((n) => (
            <Link key={n.id} href={`/dashboard/notes/${n.id}`} className={noteItemClass(isActive(`/dashboard/notes/${n.id}`))}>
              <FileText className="h-3.5 w-3.5 shrink-0" />
              <span className="truncate">{n.title || "Untitled"}</span>
            </Link>
          ))}
        ...
        <div className="mb-6">
          ...
          {rootLists.map((t) => (
            <Link key={t.id} href={`/dashboard/todos/${t.id}`} className={noteItemClass(isActive(`/dashboard/todos/${t.id}`))}>
              <ListTodo className="h-3.5 w-3.5 shrink-0" />
              <span className="truncate">{t.title}</span>
            </Link>
          ))}
        ...
        <div className="mb-6">
          <Link href="/dashboard/files" className={`flex items-center gap-2 px-2 py-1.5 rounded-md font-medium transition-colors ${isActive("/dashboard/files") ? "bg-[var(--active)]" : "hover:bg-[var(--hover)]"}`}>
            <File className="h-3.5 w-3.5" /> Files
          </Link>
          <Link href="/dashboard/schedule" className={`flex items-center gap-2 px-2 py-1.5 rounded-md font-medium transition-colors ${isActive("/dashboard/schedule") ? "bg-[var(--active)]" : "hover:bg-[var(--hover)]"}`}>
            <Calendar className="h-3.5 w-3.5" /> Schedule
          </Link>
          {profile?.role === "admin" && (
            <Link href="/admin" className={`flex items-center gap-2 px-2 py-1.5 rounded-md font-medium transition-colors ${isActive("/admin") ? "bg-[var(--active)]" : "hover:bg-[var(--hover)]"}`}>
